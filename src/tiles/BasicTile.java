...
public class BasicTile {
	...
	protected Interface[] interf = new Interface[6];

	/*
	 * = { new Interface(IO.INPUT, Device.CONST0), new Interface(IO.INPUT,
	 * Device.CONST0), new Interface(IO.INPUT, Device.CONST0), new
	 * Interface(IO.INPUT, Device.CONST0), new Interface(IO.INPUT,
	 * Device.CONST0), new Interface(IO.INPUT, Device.CONST0) };;
	 */
	...
	//returns position of logic interfaces
	public ArrayList<Integer> getGates() {
		return getGates(IO.BOTH);
	}
	
	//returns position of logic interfaces with a certain direction
	public ArrayList<Integer> getGates(IO dir) {
		...
		for (int i = 0; i < 6; i++) {
			...
			if ((io == dir || io == IO.BOTH) && device != Device.CORE && device != Device.WIREA
